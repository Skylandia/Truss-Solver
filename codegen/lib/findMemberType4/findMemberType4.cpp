...
void findMemberType4(const coder::c_table *edgesTable, double safteyFactor,
                     cell_wrap_6 beamType_data[], int beamType_size[1],
                     ...
                     int safteyFactor_Members_size[1], boolean_T *isPossible)
{
  ...
  coder::matlab::internal::coder::tabular::private_::b_varNamesDim r12;
  coder::table compressionTable;
  coder::array<cell_wrap_4, 1U> b_dataj;
  coder::array<cell_wrap_6, 1U> b_b_dataj;
  ...
  for (i1 = 0; i1 < i; i1++) {
    beamType_data[i1].f1.size[0] = 1;
    beamType_data[i1].f1.size[1] = 10;
    for (loop_ub = 0; loop_ub < 10; loop_ub++) {
      beamType_data[i1].f1.data[loop_ub] =
          edgesTable->data.f2.data[i1].f1[loop_ub];
    }
  }
  ...
  *isPossible = true;
  b_i = 0;
  exitg1 = false;
  while ((!exitg1) && (b_i <= edgesTable->data.f4.size[0] - 1)) {
    if (edgesTable->data.f4.data[b_i] == 0.0) {
      ...
      b_i++;
    } else if (edgesTable->data.f4.data[b_i] < 0.0) {
      double eqivTypeOne;
      int c_i;
      ...
      eqivTypeOne =
          std::abs(edgesTable->data.f4.data[b_i]) /
          (758228.0 *
           rt_powd_snf(1000.0 * edgesTable->data.f5.data[b_i], -2.061) *
           safteyFactor);
      for (c_i = 0; c_i < 6; c_i++) {
        rowIndices[c_i] = (t0_f2[c_i] > eqivTypeOne);
      }
      coder::matlab::internal::coder::tabular::private_::b_rowNamesDim::
          subs2inds(rowIndices, &numRowIndices, &a__1);
      coder::matlab::internal::coder::tabular::private_::b_varNamesDim::
          subs2inds(&r12);
      coder::matlab::internal::coder::tabular::private_::b_varNamesDim::
