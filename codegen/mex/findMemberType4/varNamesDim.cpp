    ...
    ::coder::array<real_T, 2U> &numericIndices)
{
  ...
  numericIndices.set_size(&c_emlrtRTEI, sp, 1, numericIndices.size(1));
  if (nz < 0) {
    emlrtNonNegativeCheckR2012b(static_cast<real_T>(nz), &emlrtDCI,
                                (emlrtCTX)sp);
  }
  numericIndices.set_size(&c_emlrtRTEI, sp, numericIndices.size(0), nz);
  ...
  if (logicalIndices[0]) {
    if (1 > nz) {
      emlrtDynamicBoundsCheckR2012b(1, 1, 0, &fb_emlrtBCI, (emlrtCTX)sp);
    }
  ...
  if (logicalIndices[1]) {
    if (count > nz) {
      emlrtDynamicBoundsCheckR2012b(count, 1, nz, &gb_emlrtBCI, (emlrtCTX)sp);
    }
