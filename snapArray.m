...
loadZone = parallel.pool.Constant(loadZone);
restrictedZoneArray = parallel.pool.Constant(restrictedZoneArray);
if ~iscell(trussArray)
    error("Not a cell array, use thanosTruss")
else
    outBoundArray = cell(1,halfArrayLength);
    parfor i = 1:(halfArrayLength)
        outBoundArray{i} = trussArray{i}.mutateTruss2(loadZone, restrictedZoneArray);
    end
