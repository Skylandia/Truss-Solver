function [tensionArray] = tensionCalculator4(nodesX,nodesY,numNodes,numEdges,endNodes, weightMagnitude, weightNode)
%tensionCalculator4 Now with GPU Support!
...
nodeEquationArray=zeros(numNodes*2,numEdges+3,'gpuArray');

xEquationArray = zeros(numNodes,numEdges,'gpuArray');
yEquationArray = zeros(numNodes,numEdges,'gpuArray');
...
forceArray=zeros(numNodes*2,1,'gpuArray');
...
% GPU Stuff
nodesY_GPU = gpuArray(nodesY);
nodesX_GPU = gpuArray(nodesX);
% For every node
parfor i=1:numNodes
    
    tempX = zeros(1,numEdges,'gpuArray');
    tempY = zeros(1,numEdges,'gpuArray');
    yVectorLoop = nodesY_GPU;
    xVectorLoop = nodesX_GPU;
    [row,~] = find(endNodes==i);
